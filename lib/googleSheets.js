...
const SHEET_ID = (process.env.GOOGLE_SHEET_ID || '').trim().replace(/^"|"$/g, '');
...
export async function updateRows(updates) {
    ...
    const data = updates.map(update => ({
        ...
        range: `工作表1!H${update.rowNumber}:J${update.rowNumber}`,
 ...
 * If the sheet doesn't exist or is empty, returns default values.
 */
export async function getSettings() {
    if (!SHEET_ID) return { watermark_enabled: 'true', watermark_text: '僅供 XX 旅遊辦理簽證使用' };

    const sheets = getSheetsClient();
    try {
        const response = await sheets.spreadsheets.values.get({
            spreadsheetId: SHEET_ID,
            range: 'Settings!A2:B10', // Assuming Settings sheet exists with A=Key, B=Value
        });

        const rows = response.data.values;
        const settings = {
            watermark_enabled: 'true', // Default values
            watermark_text: '僅供 XX 旅遊辦理簽證使用'
        };
        ...
        console.warn('⚠️ Settings sheet not found or error, using defaults:', error.message);
        // If sheet doesn't exist, we might want to create it, but for now just return defaults
        return { watermark_enabled: 'true', watermark_text: '僅供 XX 旅遊辦理簽證使用' };
    }
}

/**
 * Updates system settings in the "Settings" sheet.
 * This is a simple implementation that overwrites known rows.
 */
export async function updateSettings(settings) {
    ...
    // We'll use a fixed structure for simplicity: 
    // row 2: watermark_enabled
    // row 3: watermark_text
    const values = [
    ...
    try {
        ...
        console.log('✅ Google Sheets Settings Update Success');
    } catch (error) {
